...
def pipeline_lng_classification(patient):
    ...
    for i, tp in patient.items():
        if not os.path.exists(tp.stx2_mnc['lng_classification']) or not os.path.exists(tp.qc_jpg['lngclassification']):
            allDone = False
            break
...
def classify_prior(inputs, output, mask=None, model_dir=None, model_name=None, xfm=None, priors=None, mrf=False):
    with mincTools() as minc:
        ...
        argsclass = [
            ...
            '-apriori',
            '-volume',
            "{},{},{}".format(priors[0], priors[1], priors[2]),
            '-tagfile',
            cleantags,
        ]
    ...
    with mincTools() as minc:
        ...
        if lng_cls == 'doGC4D':
            ...
            for i in range(len(lngclassif)):
                minc.calc(
                    [gcut2[i], gcut1[i], masks[i]],
                    'if(A[0]>0.5){3}else if(A[1]>0.5){2} else if(A[2]>0.5){1} else{0}',
                    lngclassif[i],
                )
            ...
            for i, tp in patient.items():
                ...
                if not patient.onlyt1 and 't2' in tp.stx2_mnc and os.path.exists(tp.stx2_mnc['t2']):
                    t2.append(tp.stx2_mnc['t2'])

                if not patient.onlyt1 and 'pd' in tp.stx2_mnc and os.path.exists(tp.stx2_mnc['pd']):
